...
def configure_courses_controller():
    controller = Controller()

    @controller.action('save_courses: Save a list of courses to a file', param_model=CourseList)
    def save_courses(course_list: CourseList):
        courses_file = f"out/courses_{to_valid_filename(course_list.department_name)}.json"

        print(f"save_courses: {course_list.model_dump_json(indent=2)}")

        with open(courses_file, "w") as f:
            f.write(course_list.model_dump_json(indent=2))
